...
export default function NoteCards() {
    ...
    const handleDeleteNoteCard = async (uniqueID) => {
        try {
            const token = localStorage.getItem('authToken');
            await axios.delete(`http://localhost:8001/delete-noteCard?uniqueID=${uniqueID}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleDeleteQuestion = async (noteCardUnique, questionId) => {
        try {
            ...
            await axios.delete(`http://localhost:8001/delete-noteCard-question?noteCardUnique=${noteCardUnique}&questionId=${questionId}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleSaveEdit = async () => {
        try {
            const token = localStorage.getItem('authToken');
            await axios.patch(`http://localhost:8001/edit-noteCard-question`, {
